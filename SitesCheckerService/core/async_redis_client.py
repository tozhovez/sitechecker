...
from msgpack_serializer import packb, unpackb
...
class RedisClient(object):
    def __init__(self, addr: str, port: int = 6379, db: int = 0):
        ...
        self.port = port
        self.db = db
        self.client = AsyncRedisClient(addr, port, db)

    async def connect(self):
        await self.client.connect()

    async def set(self, key, value, expire: float = 60 * 10):
        try:
            await self.client.set(key, value, expire)
        except Exception as e:
            logger.error('Error setting value in redis', exc_info=True, extra={
                'key': key,
                'value': value
            })

    async def get(self, key: float):
        res = None
        ...
        try:
            ...
            logger.error('Error getting value from redis', exc_info=True, extra={
                'key': key,
            })
...
class AsyncRedisClient(object):
    ...
    @retry(stop=stop_after_attempt(5), retry=retry_if_exception_type(CONN_ERRORS), wait=wait_fixed(5))
    async def connect(self):
        self.client = await aioredis.create_redis_pool(self.url, timeout=5, maxsize=1)
    ...
    @retry(stop=stop_after_attempt(5), retry=retry_if_exception_type(CONN_ERRORS), wait=wait_fixed(2))
    ...
    @retry(stop=stop_after_attempt(5), retry=retry_if_exception_type(CONN_ERRORS), wait=wait_fixed(2))
    async def get_by_key_pattern(self, pattern: str, block_server: bool = True):
        result = []
