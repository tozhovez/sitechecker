...
class ConsulClient(object):
    ...
    @retry(retry=retry_if_exception_type(ConnectionError), stop=stop_after_delay(RETRY_DELAY_IN_SECONDS))
    def get(self, key, prefix=None):
        try:
            _key = self.prefixed_key(prefix, key)
            _, data = self.consul_client.kv.get(_key)
            if not data:
                raise ConnectionError('Key: {0} is not found'.format(key))
    ...
    def get_all(self):
        try:
            ...
            for item in data:
                only_kv_data.append({"Key": item['Key'], "Value": json.loads((item['Value'].decode('utf-8')))})
            return only_kv_data
        except ConnectionError as ex:
            self.connect()
            raise ex
